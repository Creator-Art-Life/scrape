...
export default function ExecutionViewer({
  ...
  useEffect(() => {
    // Получаем список фаз из данных запроса или создаем пустой массив, если данных нет
    const phases = query.data?.phases || [];

    // Если процесс запущен
    if (isRunning) {
      // Выбираем последнюю запущенную фазу
      const phaseToSelect = phases.toSorted((a: any, b: any) =>
        a.startedAt > b.startedAt ? -1 : 1
      )[0];

      // Устанавливаем выбранную фазу
      setSelectedPhase(phaseToSelect.id);
      return;
    }

    // Если процесс не запущен, выбираем последнюю завершенную фазу
    const phaseToSelect = phases.toSorted((a: any, b: any) =>
      ...
      <CardContent className="py-4">
        <div className="flex flex-col gap-2">
          {(!params || Object.keys(params).length === 0) && (
            <p className="text-sm">No paraments generated by this phase</p>
          )}
          {params &&
